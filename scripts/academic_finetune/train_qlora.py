...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        description="Pinned, completion-only MLX QLoRA for Spiral academic prose")
    result.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    result.add_argument("--corpus", type=Path, help="combined corpus JSONL from build_corpus.py")
...
def _prepare(arguments: argparse.Namespace, config: dict) -> dict:
    if arguments.corpus is not None:
        return prepare_mlx_dataset(
            ...
            require_trainable=arguments.smoke_model is None)
    return load_dataset_manifest(arguments.data_dir)
...
def _production(arguments: argparse.Namespace, config: dict, dataset: dict) -> int:
    ...
    previous = latest if arguments.resume else None
    ...
    if previous is not None and completed_steps == total_steps:
        ...
        _write_training_status(
            ...
            schedule=schedule, resume_command=resume_command, total_steps=total_steps,
            checkpoint=previous)
    ...
    if previous is not None:
        atomic_write_bytes(
            work_adapter / "adapter_config.json", previous.config_path.read_bytes())
    ...
    atomic_write_bytes(
        ...
        yaml_training_config(
            ...
            resume_adapter=previous.path if previous else None,
        ).encode("utf-8"),
    )
    command = mlx_training_command(yaml_path, python_executable=arguments.python)
    ...
    _write_training_status(
        ...
        schedule=schedule, resume_command=resume_command, total_steps=total_steps,
        checkpoint=previous, attempt_id=attempt_id)
    if not arguments.execute:
    ...
    try:
        ...
        with lease:
            verify_ollama_empty(arguments.ollama_url)
            result = run_training_process(
                command, work_adapter, ledger, cumulative_offset=completed_steps,
        ...
        _write_training_status(
            ...
            schedule=schedule, resume_command=resume_command, total_steps=total_steps,
            checkpoint=durable, attempt_id=attempt_id, error=str(exc))
        print(f"training interrupted; durable checkpoint step: {durable.step if durable else 0}",
              file=sys.stderr)
    ...
    if result:
        ...
        _write_training_status(
            ...
            schedule=schedule, resume_command=resume_command, total_steps=total_steps,
            checkpoint=durable, attempt_id=attempt_id,
            error=f"MLX-LM trainer exited with status {result}")
        print(f"training failed; durable checkpoint step: {durable.step if durable else 0}",
              file=sys.stderr)
        print(f"resume with: {resume_command}", file=sys.stderr)
        raise HarnessError(f"MLX-LM trainer exited with status {result}")
    try:
        ...
        _write_training_status(
            ...
            schedule=schedule, resume_command=resume_command, total_steps=total_steps,
            checkpoint=durable, attempt_id=attempt_id, error=str(exc))
        print(f"finalization interrupted; resume with: {resume_command}", file=sys.stderr)
        raise
    _write_training_status(
        arguments.output, state="completed", run_contract=run_contract,
        schedule=schedule, resume_command=resume_command, total_steps=total_steps,
        checkpoint=completed, attempt_id=attempt_id)
